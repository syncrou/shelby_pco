...
def transfer(str)
  if str == 'Trans from CRC'
    'Transfer from CRC'
  elsif str == 'Non-CRC Trans'
    'Transfer from non-CRC'
  else
    str
  end
end
  ...
  CSV.open('pco_output.csv', 'wb') do |csv|
    ...
    CSV.foreach(filename, :encoding => 'ISO-8859-1') do |row|
      ...
      csv << [household_ids(row[headers.index("FamilyMember1NameID")], row[103], row[116], row[129], row[142], row[155], row[168], row[181], row[194], row[207]),
              ...
              city(row[headers.index("City")]), state(row[headers.index("State")]), zip(row[headers.index("PostalCode")]), phone(row[headers.index("Phone#")]), '',row[headers.index("Envelope#")], mail_status(row[headers.index("Envelope#")]), row[headers.index("FamilyPosition")],
              row[headers.index("EmailAddress")], row[headers.index("WebAddress")], transfer(row[headers.index("How Received")]), convert_dates(row[headers.index("Date Received")]), row[headers.index("Relationship Status")], convert_dates(row[headers.index("DateBaptism")]), convert_dates(row[headers.index("DateProfession")]), primary(row[headers.index("MailStatus")]), row[headers.index("PrintStatements")]] unless shelby_cnt == 0
      shelby_cnt += 1
    end
  end
end
